...
import { useUserStore } from '../../stores/userStore.js'; // 引入全局状态

// 引入子页面组件
import OrderManagement from '../settingsSonPage/OrderManagement.vue';
import SalesRecord from '../settingsSonPage/SalesRecord.vue';
import PaidPromotion from '../settingsSonPage/PaidPromotion.vue';
import Message from '../settingsSonPage/Message.vue';
import MyCustomerService from '../settingsSonPage/MyCustomerService.vue';
import MySettings from '../settingsSonPage/MySettings.vue';
import MyRights from "../settingsSonPage/MyRights.vue"; // 引入设置组件
import DeviceManage from '../settingsSonPage/DeviceManage.vue';
...
const userName = computed(() => {
  ...
  if (userStore.isCustomer){
    // 将原本的批发商替换为店面，在这里添加了批发商的判断逻辑
    if(userStore.isWholesalers2){
      result = "xxxxxxx批发商"
    }
    else {
      result = "xxxxxxx消费者"
...
const filteredFunctions = computed(() => {
  ...
  if(userStore.isFamer){
    return [
      ...
      { icon: 'mdi:chart-line', label: '销售记录' },
      { icon: 'mdi:bullhorn', label: '付费推广' },
      { icon: 'material-symbols:assistant-device-outline', label: '我的设备' },
      { icon: 'mdi:headset', label: '我的客服' },
      { icon: 'mdi:cog', label: '我的设置' },
    ]
  }else if(userStore.isWholesalers){
...
const showPage = (label) => {
  switch (label) {
    ...
    case '我的权益':
      ...
      break;
    case '我的设备':
      currentPage.value = DeviceManage;
    default:
      break;
  }
};
